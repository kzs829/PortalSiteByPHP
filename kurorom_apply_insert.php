...
foreach($app_details as $row_data){
  kurorom_apply_insert($pdo, $row_data, $uuid);
}

//申込管理情報をDBに格納
applicant_info_insert($pdo, $applicant_info, $uuid);
...
<?php
function kurorom_apply_insert($pdo, $row_data, $uuid){
  $err_message = null;
  try{
    $tsql = "INSERT INTO kurorom_apply (apply_id, category, site, item_number, later_site, new_item_number, item_name) VALUES (?, ?, ?, ?, ?, ?, ?);";
    $stmt = $pdo->prepare($tsql);
    $params = array($uuid, $row_data[0], $row_data[1], $row_data[2], $row_data[3], $row_data[4], $row_data[5]);
    $stmt->execute($params);
...
function applicant_info_insert($pdo, $applicant_info, $uuid){
  ...
  try{
    $tsql = "INSERT INTO application (apply_id, type, department, name, mail, apply_date, delivery_date, reseption_department) VALUES (?, '黒ROMマスタ登録依頼', ?, ?, ?, ?, ?, 'SC戦略生産革新部');";
    $stmt = $pdo->prepare($tsql);
